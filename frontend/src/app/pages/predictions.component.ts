          ...
          @for (match of filtered(); track match.fixture_id) {
            <article class="prediction-card card">
              ...
              <div class="prediction-teams">
                <div>
                  <app-team-logo [teamName]="match.home_team" [size]="42" />
                  <strong>{{ match.home_team }}</strong>
                </div>
                <span class="predicted-score">{{ match.predicted_score }}</span>
                <div>
                  <app-team-logo [teamName]="match.away_team" [size]="42" />
...
export class PredictionsComponent {
  ...
  private readonly selectedMatchday = signal(1);
  ...
  readonly filtered = computed(() => {
    ...
    return this.predictions().filter(
      (match) =>
        (matchday === 0 || match.matchday === matchday) &&
        (!query ||
  ...
  set matchday(value: number) {
    this.selectedMatchday.set(Number(value));
  }
