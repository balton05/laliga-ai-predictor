...
export class TeamsComponent {
  ...
  readonly cards = computed<TeamCard[]>(() => {
    const standings = new Map(
      this.standings().map((row) => [row.team_id, row] as const),
    );
    const simulations = new Map(
      this.simulation().map((row) => [row.team_id, row] as const),
    );
    return TEAM_ASSETS.map((team) => ({
      ...team,
      standing: standings.get(team.teamId),
      simulation: simulations.get(team.teamId),
    })).sort(
      (a, b) =>
